...
<?php
	...
	if(!empty($_COOKIE['username'])) {
	  $username = $_COOKIE['username'];
		$forums = mysqli_query($dbc, "SELECT * FROM `comments`");
	}


	if(isset($_GET['id']) && (preg_match("/^[0-9]+$/i", $_GET['id']))) {
		$forum_id = $_GET["id"];
		$exactForum = mysqli_query($dbc, "SELECT * FROM `comments` WHERE forum_id = '$forum_id'");
...
<main>
		...
		<a  href="add_comment.php" class="createForumBtn">Create new discussion</a>
		...
		<button class="eachForum" id="<?php echo $rowForums['forum_id']; ?>" name="forumBtn" onclick="showForum(this.id)">
			...
			<?php
				...
				if (mysqli_num_rows($exactForum)) {
			?>

		<a href="comments.php" class="goBack"><i class="fas fa-long-arrow-left"></i> Go back</a>
