...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	if (isset($_POST['forum_id'])) {
		...
		$date = date('Y-m-d');
		$answer_message = nl2br($_POST['answer']);
		$username = $_POST['username'];
		mysqli_query($dbc, "INSERT INTO `messages`(`forum_id`, `from_user`, `date`, `message`) VALUES ('$forum_id','$username','$date','$answer_message')");

		mysqli_query($dbc, "UPDATE `comments` SET `num_of_comments`= ((SELECT `num_of_comments` FROM forums WHERE forum_id = '$forum_id') + 1) WHERE forum_id = '$forum_id'");
	...
	if (isset($_POST['number_of_answers'])) {
		$forum_id = $_POST['f_id'];
		$numOfAnswers = mysqli_query($dbc, "SELECT `num_of_comments` FROM `comments` WHERE forum_id = '$forum_id'");
